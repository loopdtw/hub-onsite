...
noble.on('discover', function(peripheral) {
    ...
    //updating seen badges and broadcast to frontend
    // updateSeenBadges(badge);
...
function disconnectPeripheral(peripheral, message) {
    logger.info("peripheral disconnect: " + message);

    try {
        peripheral.disconnect(function(error) {
            logger.info('peripheral forcably disconnected!');
        });
    } catch (err) {
        logger.error(peripheral.advertisement.localName, err);
        logger.error(peripheral.advertisement.localName, err.stack);
    }
}

function updateSeenBadges(badge) {
    //updating seen badges
    if(typeof seenBadges[badge.macAddress] == 'undefined') {
        seenBadges[badge.macAddress] = badge;
    } else {
        var existingBadge = seenBadges[badge.macAddress];
        existingBadge.lastSeen = new Date();
        ...
        existingBadge.incrementAdCount();
        existingBadge.calculateAdRate();
    }
}
