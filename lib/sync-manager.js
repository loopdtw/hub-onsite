...
exports.commandBadge = function(badge, badgeCommand) {
    return Promise.try(function() {
       if(!seenPeripherals[badge.identity + badge.macAddress]) {
            throw Common.errorFormatter('Badge not found', 400);
        } else if (seenPeripherals[badge.identity + badge.macAddress].commandStatus === peripheralCommandStatus.STATUS_COMMANDING) {
            throw Common.errorFormatter('Badge is still commanding', 400);
        } else {
            SocketManager.broadcastStatus('Connecting to badge ' + badge.identity);
