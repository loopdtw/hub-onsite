            ...
            db.get(peripheral.address, function (err, value) {
                if (err) {
                    ...
                    if (typeof existingBadges[peripheral.getIdMac()] === 'undefined' &&
                        ...
                        peripheral.status == peripheralStatus.STATUS_NONE &&
                        connectedPeripherals.length < 1) {
                        connectedPeripherals.push(peripheral.advertisement.localName);
    ...
    peripheral.connect(function(error) {
        if (!error) {
            if (mode === connectBadgeMode.COMMAND) {
                ...
                allocateBadge(peripheral);
            } else if (mode === connectBadgeMode.UNSYNC) {
                unsyncBadge(peripheral);
            }
        } else {
...
function issueCommand(peripheral) {
    ...
    if (badgeCommands[index]) {
        ...
        var writeStart = new Date().getTime() / 1000;
        peripheral.writeHandle(0x2a, badgeCommands[index], false, function(error) {
                if(!error) {
                    logger.info(peripheral.advertisement.localName, 'write success!', (new Date().getTime() / 1000) - writeStart + "s");
...
function syncBadge(peripheral) {
    peripheral.writeHandle(0x2a, new Buffer([0xd5]), false); //setting to in-event mode
    peripheral.writeHandle(0x2a, new Buffer([0x07]), false); //clearing contacts
    peripheral.writeHandle(0x2a, new Buffer([0xf7, 0x03, 0x03]), false); //blinking LEDs
    allocatedPeripherals.del(peripheral.address);
    ...
    peripheral.once('handleNotify', function(handle, data) {
        ...
        if (data[1] == 0xd5) {
            ...
            peripheral.writeHandle(0x2a, new Buffer([0x11]), false); //clearing contacts
        }
    });
}
...
function unsyncBadge(peripheral) {
    peripheral.writeHandle(0x2a, new Buffer([0xd3]), false);
    peripheral.writeHandle(0x2a, new Buffer([0x07]), false);
    peripheral.writeHandle(0x2a, new Buffer([0xa0, 0x08]), false);
...
function allocateBadge(peripheral) {
    peripheral.writeHandle(0x2a, new Buffer([0xa0, 0x08]), false);
    peripheral.writeHandle(0x2a, new Buffer([0x11]), false);
    peripheral.once('handleNotify', function(handle, data) {
        logger.info("notified raw data: ", data);
