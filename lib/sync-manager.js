...
function _syncBadge(peripheral) {
    peripheral.writeHandleAsync(0x2a, new Buffer([0x07]), false).then(function() {
        ...
        existingBadges[peripheral.address] = peripheral;
...
function _updateInfoForBadge(peripheral) {
    ...
    SocketManager.broadcastStatus('Uploading data for attendee: ' + currentAttendee.firstname);
    LoopdBadge.syncToAttendee(currentAttendee, peripheral).then(function(attendee) {
        ...
        SocketManager.broadcastStatus('Successfully synced attendee ' + attendee.firstname + ' for badge ' + peripheral.advertisement.localName + ' (BLE Sync: ' + syncDuration.toFixed(2) + 's, Upload: ' + uploadDuration.toFixed(2) + 's)');
        ...
        logger.error(err);
        if(existingBadges[peripheral.address]) delete existingBadges[peripheral.address];
        SocketManager.broadcastStatus('Error syncing badge ' + err);
...
function _handleDiscover(peripheral) {
    peripheral = Promise.promisifyAll(peripheral);
    ...
    if (peripheral.rssi > -50) {
        logger.info(peripheral.advertisement.localName, peripheral.rssi);
        return enableLookup?
...
exports.updateExistingBadges = function(badges) {
    ...
    badgeKeys.forEach(function(badgeKey) {
